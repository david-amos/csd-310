...
def show_equipment_sales(cursor,title):
    ...
    #inner join query
    cursor.execute("select ItemDescription, COUNT(*), SupplyPricetoBuy from Supply where supplystatus = 'bought' group by ItemDescription, SupplyPricetoBuy")
    # get results
...
def show_booking_trends(cursor,title):
    ...
    #inner join query
    cursor.execute("select Destination, YEAR(DepartureDate), MONTH(DepartureDate), Count(*) from Trips Group by Destination, YEAR(DepartureDate), MONTH(DepartureDate) Order by Destination, YEAR(DepartureDate), MONTH(DepartureDate)")
    # get results
    ...
    #interate over results to display
    for trip in trips:
        print("Location: {}\nDeparture Year: {}\nDeparture Month: {}\nNumber of Bookings: {}\n".format(trip[0], trip[1], trip[2], trip[3]))

def show_old_equipment(cursor,title):
    #method to execute query and output results

    #inner join query
    cursor.execute("select SuppliesID, ItemDescription, AcquiredDate from Supply WHERE YEAR(CURDATE()) - YEAR(AcquiredDate) > 5")
    # get results
    supplies = cursor.fetchall()
